...
export class NavService extends AbstractService<Menu>  {
	...
	constructor(
		...
		super(_httpClient);
		let menuItems: Array<Menu> = new Array<Menu>();
		this.items = new BehaviorSubject<Menu[]>(menuItems);
		this.getMenu().then((response) => {

			menuItems.push({ title: 'HEADER_MENU.HOME', type: 'link', active: false, path: '/home' });
			response.data.menu.forEach((menu: Menu) => {
				switch (menu.title) {
					case 'SHOP':
						menu.title = 'HEADER_MENU.' + menu.title
						menu.children.forEach((serviceType: Menu) => {
							serviceType.children.forEach((services: Menu) => {
								services.path = '/shop/services/'+ services.path;
							});
						});
						menuItems.push(menu);
						break;
			...
			menuItems.push({ title: 'HEADER_MENU.HOME', megaMenu: true, type: 'link', active: false, path: '/home' });
		});
	}
	...
	MENUITEMS: Menu[] = [
		{
			title: 'HEADER_MENU.HOME', type: 'link', active: false, path: '/home'
		},
		{
			title: 'HEADER_MENU.SHOP', type: 'link', active: false, path: '/shop/collection'
		},
		{
			title: 'Product', type: 'link', active: false, path: '/shop/products/trim-dress'
		},
		/* 	{
		...
		},
		{
			title: 'blogs', type: 'sub', active: false, children: [
				{ path: '/pages/blog', title: 'left-sidebar', type: 'link' },
	...
	getMenu(): Promise<any> {
		...
		return this.search(this.apollo, 'services', query);
	}
